...
async fn join(user: &mut GooseUser) -> TransactionResult {
    ...
    match client.login_username(username, password).send().await {
        Ok(_) => {
            ...
            println!("[{}] has {} pending invites", username, client.invited_rooms().len());
            ...
            for invite in client.invited_rooms() {
                ...
                while retries > 0 {
                    match client.join_room_by_id(invite.room_id()).await {
                        ...
                        Err(err) => {
                            println!("[{}] Could not join room {} (attempt {}): {:?}. Trying again...",
                                username, invite.room_id(), 4 - retries, err);
                            retries -= 1;

                            if retries == 0 {
                                println!("[{}] Error joining room {}. Skipping...", username, invite.room_id());
                                break;
                            }
                        },
                    }
