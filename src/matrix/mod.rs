...
use goose::{prelude::*, goose::{GooseResponse, TransactionError}, metrics::GooseRequestMetric};
use matrix_sdk::{
...
type NotificationHandlerFn =
    ...
    Box<dyn Fn(Notification, room::Room, GooseMatrixClient) -> NotificationHandlerFut + Send + Sync>;
#[cfg(target_arch = "wasm32")]
type NotificationHandlerFn =
    // Box<dyn Fn(Notification, room::Room, Client) -> NotificationHandlerFut>;
    Box<dyn Fn(Notification, room::Room, GooseMatrixClient) -> NotificationHandlerFut>;
...
pub(crate) struct ClientInner {
    ...
    /// Index of the user in the static table of Goose user references. The
...
// #[derive(Debug, Clone)]
#[derive(Debug)]
pub struct MatrixError<T> {
    pub error: Option<T>,
    pub goose_error: Option<Box<TransactionError>>,
}

#[cfg(not(tarpaulin_include))]
// impl Debug for Client {
impl Debug for GooseMatrixClient {
...
impl From<HttpError> for MatrixError<HttpError> {
    fn from(value: HttpError) -> Self {
        MatrixError { error: Some(value), goose_error: None }
    }
}

impl From<IntoHttpError> for MatrixError<HttpError> {
    fn from(value: IntoHttpError) -> Self {
        MatrixError { error: Some(HttpError::IntoHttp(value)), goose_error: None }
    }
}

impl From<RefreshTokenError> for MatrixError<HttpError> {
    fn from(value: RefreshTokenError) -> Self {
        MatrixError { error: Some(HttpError::RefreshToken(value)), goose_error: None }
    }
}

impl From<reqwest::Error> for MatrixError<HttpError> {
    fn from(value: reqwest::Error) -> Self {
        MatrixError { error: Some(HttpError::Reqwest(value)), goose_error: None }
    }
}


impl From<Box<TransactionError>> for MatrixError<Box<TransactionError>> {
    fn from(value: Box<TransactionError>) -> Self {
        MatrixError { error: None, goose_error: Some(value) }
    }
}

impl From<IntoHttpError> for MatrixError<IntoHttpError> {
    fn from(value: IntoHttpError) -> Self {
        MatrixError { error: Some(value), goose_error: None }
    }
}

impl From<MatrixError<HttpError>> for ClientBuildError {
    fn from(value: MatrixError<HttpError>) -> Self {
        ClientBuildError::Http(value.error.unwrap())
    }
}

//crate::matrix::error::Error
impl From<Error> for MatrixError<Error> {
    fn from(value: Error) -> Self {
        MatrixError { error: Some(value), goose_error: None }
    }
}

impl From<MatrixError<Error>> for Error {
    fn from(value: MatrixError<Error>) -> Self {
        value.error.unwrap()
    }
}

impl From<MatrixError<HttpError>> for Error {
    fn from(value: MatrixError<HttpError>) -> Self {
        value.error.unwrap().into()
    }
}

impl From<matrix_sdk_base::Error> for MatrixError<Error> {
    fn from(value: matrix_sdk_base::Error) -> Self {
        MatrixError { error: Some(value.into()), goose_error: None }
    }
}

impl From<MatrixError<HttpError>> for MatrixError<Error> {
    fn from(value: MatrixError<HttpError>) -> Self {
        MatrixError { error: Some(Error::Http(value.error.unwrap())), goose_error: None }
    }
}

impl From<StoreError> for MatrixError<Error> {
    fn from(value: StoreError) -> Self {
        MatrixError { error: Some(Error::StateStore(value)), goose_error: None }
    }
}

impl From<serde_json::Error> for MatrixError<Error> {
    fn from(value: serde_json::Error) -> Self {
        MatrixError { error: Some(Error::SerdeJson(value)), goose_error: None }
    }
}

impl From<ruma::IdParseError> for MatrixError<Error> {
    fn from(value: ruma::IdParseError) -> Self {
        MatrixError { error: Some(Error::Identifier(value)), goose_error: None }
    }
}


// impl Client {
impl GooseMatrixClient {
    ...
    pub async fn new(homeserver_url: Url, goose_user_index: usize) -> Result<GooseMatrixClient, HttpError> {
        // Self::builder()
        Self::builder(goose_user_index)
    ...
    pub fn builder(goose_user_index: usize) -> GooseClientBuilder {
        // ClientBuilder::new()
        GooseClientBuilder::new(goose_user_index)
    }
    ...
    // ) -> Result<MatrixResponse<search_users::v3::Response>, MatrixError<Error>> {
    ) -> Result<MatrixResponse<search_users::v3::Response>, MatrixError<HttpError>> {
    ...
    where
        ...
        H: Fn(Notification, room::Room, GooseMatrixClient) -> Fut
            + SendOutsideWasm
            + SyncOutsideWasm
            + 'static,
    ...
    async fn receive_login_response(&self, response: &login::v3::Response) -> Result<(), Error> {
    ...
    pub async fn restore_session(&self, session: Session) -> Result<(), Error> {
    ...
    pub async fn refresh_access_token(&self) -> Result<MatrixResponse<Option<refresh_token::v3::Response>>, MatrixError<HttpError>> {
        ...
        if let Ok(mut guard) = lock {
            let Some(mut session_tokens) = self.session_tokens() else {
                *guard = Err(RefreshTokenError::RefreshTokenRequired);
                ...
                return Err(MatrixError { error: Some(RefreshTokenError::RefreshTokenRequired.into()), goose_error: None });
            };
            ...
            let res = self
                ...
                .send(
                    ...
                    self.server_versions().await?.response.unwrap(),
                    self.inner.goose_user_index,
                )
                .await;
            ...
            match res {
                Ok(res) => {
                    *guard = Ok(());
                    ...
                    session_tokens.update_with_refresh_response(&response.unwrap());
                    ...
                    // Ok(Some(res))
                    Ok(MatrixResponse { request: None, response: Some(res.response) })
                }
                Err(error) => {
                    ...
                    *guard = match error.as_ruma_api_error() {
                    ...
                    // Err(error)
                    Err(MatrixError { error: Some(error), goose_error: None })
                }
            }
        } else {
            match *self.inner.refresh_token_lock.lock().await {
                    // Ok(_) => Ok(None),
                    Ok(_) => Ok(MatrixResponse { request: None, response: None }),
                    // Err(_) => Err(RefreshTokenError::UnableToRefreshToken.into()),
                    Err(_) => Err(MatrixError { error: Some(RefreshTokenError::UnableToRefreshToken.into()), goose_error: None }),
            }
        }
    }
    ...
    ) -> Result<MatrixResponse<register::v3::Response>, MatrixError<HttpError>> {
        let homeserver = self.homeserver().await;
    ...
    pub async fn join_room_by_id(&self, room_id: &RoomId) -> Result<room::Joined, Error> {
    //) -> Result<MatrixResponse<join_room_by_id::v3::Response>, MatrixError<HttpError>> {
        let request = join_room_by_id::v3::Request::new(room_id.to_owned());
        // let response = self.send(request, None).await?;
        let response = self.send(request, None).await?.response.unwrap();
    ...
    // ) -> Result<room::Joined> {
    ) -> Result<room::Joined, Error> {
    //) -> Result<MatrixResponse<join_room_by_id_or_alias::v3::Response>, MatrixError<HttpError>> {
    ...
    ) -> Result<MatrixResponse<get_public_rooms::v3::Response>, MatrixError<HttpError>> {
    ...
    pub async fn create_room(&self, request: create_room::v3::Request) -> Result<room::Joined, Error> {
    //) -> Result<MatrixResponse<create_room::v3::Response>, MatrixError<HttpError>> {
    ...
    pub async fn create_dm(&self, user_id: &UserId) -> Result<room::Joined, Error> {
    ...
    ) -> Result<MatrixResponse<get_public_rooms_filtered::v3::Response>, MatrixError<HttpError>> {
        self.send(request, None).await
    }
    ...
    ) -> Result<MatrixResponse<Request::IncomingResponse>, MatrixError<HttpError>>
    where
        ...
        HttpError: From<FromHttpResponseError<Request::EndpointError>>,
    {
        ...
        // TODO: Add back refresh tokens after initial testing
        ...
        //     // )))) = &res
        //     if let Err(MatrixError { error: Some(HttpError::Api(FromHttpResponseError::Server(ServerError::Known(
        //         RumaApiError::ClientApi(error),
        //     )))), goose_error: None }) = &res
        //     {
        ...
        //                 // match &refresh_error {
        //                 match &refresh_error.error {
        //                     Some(HttpError::RefreshToken(RefreshTokenError::RefreshTokenRequired)) => {
        //                         // Refreshing access tokens is not supported by
        //                         // this `Session`, ignore.
        //                     }
        //                     _ => {
    ...
    async fn request_server_versions(&self) -> Result<MatrixResponse<Box<[MatrixVersion]>>, MatrixError<HttpError>> {
        ...
        if server_versions.is_empty() {
            ...
            Ok(MatrixResponse { request: None, response: Some(vec![MatrixVersion::V1_0].into()) })
        } else {
            // Ok(server_versions)
            Ok(MatrixResponse { request: None, response: Some(server_versions) })
        }
    }
    ...
    pub(crate) async fn server_versions(&self) -> Result<MatrixResponse<&[MatrixVersion]>, MatrixError<HttpError>> {
        let server_versions =
            // self.inner.server_versions.get_or_try_init(|| self.request_server_versions()).await?;
            self.inner.server_versions.get_or_try_init(|| async {
                // let versions: Box<impl Future<Output = Result<MatrixResponse<Box<[MatrixVersion]>>, MatrixError>>> = self.request_server_versions().into();
                // TODO: re-evaluate if possible to throw errors?
                let versions = self.request_server_versions().await.unwrap().response;
                // Ok(versions) as Result<Box<[MatrixVersion]>, MatrixError<HttpError>>
                Ok(versions.unwrap()) as Result<Box<[MatrixVersion]>, MatrixError<HttpError>>
            }).await?;
    ...
    pub async fn devices(&self) -> Result<MatrixResponse<get_devices::v3::Response>, MatrixError<HttpError>> {
    ...
    ) -> Result<MatrixResponse<delete_devices::v3::Response>, MatrixError<HttpError>> {
    ...
    ) -> Result<MatrixResponse<update_device::v3::Response>, MatrixError<HttpError>> {
    ...
    pub async fn sync_once(
        ...
        sync_settings: crate::matrix::config::SyncSettings,
    // ) -> Result<SyncResponse> {
    ) -> Result<MatrixResponse<SyncResponse>, MatrixError<Error>> {
        ...
        let response = self.send(request, Some(request_config)).await?.response.unwrap();
        ...
        Ok(MatrixResponse { request: None, response: Some(SyncResponse::new(next_batch, response)) })
    }
    ...
    // // ) -> Result<SyncResponse> {
    // // ) -> crate::matrix::error::Result<SyncResponse> {
    // ) -> Result<MatrixResponse<SyncResponse>, MatrixError<Error>> {
    //     // The sync might not return for quite a while due to the timeout.
    //     // We'll see if there's anything crypto related to send out before we
    //     // sync, i.e. if we closed our client after a sync but before the
    ...
    //     // let response = self.send(request, Some(request_config)).await?;
    //     let response1 = self.send(request, Some(request_config)).await?;
    //     // let response = self.process_sync(response).await?;
    //     let response = self.process_sync(response1.response.unwrap()).await?;
    ...
    //     // Ok(response)
    //     Ok(MatrixResponse { request: response1.request, response: Some(response) })
    // }
    ...
    pub async fn sync(&self, sync_settings: crate::matrix::config::SyncSettings) -> Result<(), Error> {
    ...
    pub async fn sync_with_result_callback<C>(
        ...
        mut sync_settings: crate::matrix::config::SyncSettings,
    ...
    ) -> impl Stream<Item = crate::matrix::error::Result<SyncResponse>> + '_ {
        let mut last_sync_time: Option<Instant> = None;
    ...
    pub async fn whoami(&self) -> Result<MatrixResponse<whoami::v3::Response>, MatrixError<HttpError>> {
    ...
    pub async fn logout(&self) -> Result<MatrixResponse<logout::v3::Response>, MatrixError<HttpError>> {
    ...
    pub async fn set_pusher(&self, pusher: Pusher) -> Result<MatrixResponse<set_pusher::v3::Response>, MatrixError<HttpError>> {
    ...
    pub async fn get_profile(&self, user_id: &UserId) -> Result<MatrixResponse<get_profile::v3::Response>, MatrixError<HttpError>> {
...
//     future::Future,
//     pin::Pin,
//     sync::Arc, ops::{Deref, DerefMut}, cell::RefCell,
// };
...
//             uiaa::{AuthData, UserIdentifier},
//         },
//         error::{FromHttpResponseError, ServerError, IntoHttpError},
//         MatrixVersion, OutgoingRequest, SendAccessToken, IncomingResponse, EndpointError,
//     },
//     assign,
...
// // comment regarding lifetimes and static goose user references as well, also that cannot store the pointer in the struct since gooseuser does not implement sync trait
// // pub static mut GOOSE_USERS: Vec<Option<&mut GooseUser>> = Vec::new();
// // pub static mut GOOSE_USERS: Vec<Option<Box<&mut GooseUser>>> = Vec::new();
// // pub static mut GOOSE_USERS: Vec<Option<Arc<&mut GooseUser>>> = Vec::new();
// // pub static mut GOOSE_USERS: Vec<Option<RefCell<GooseUser>>> = Vec::new();
// // pub static mut GOOSE_USERS: Vec<Option<RefCell<&mut GooseUser>>> = Vec::new();
// // pub static mut GOOSE_USERS: Vec<Option<Mutex<&mut GooseUser>>> = Vec::new();
// // pub static mut GOOSE_USERS: Vec<*mut &mut GooseUser> = Vec::new();
// pub static mut GOOSE_USERS: Vec<*mut GooseUser> = Vec::new();
// // pub static mut GOOSE_USERS: Vec<Box<Option<GooseUser>>> = Vec::new();
// // pub static GOOSE_USERS_READER: &Vec<Option<RefCell<&mut GooseUser>>> = unsafe { &GOOSE_USERS };
...
// // impl From<matrix_sdk::Result<()>> for MatrixError<matrix_sdk::Result<()>> {
// //     fn from(value: matrix_sdk::Result<()>) -> Self {
// //         MatrixError { error: Some(value), goose_error: None }
// //     }
// // }
...
// //From<FromHttpResponseError<Self::EndpointError>>>
// // impl<Request: EndpointError> From<FromHttpResponseError<Request>> for MatrixError {
// // impl<Request as OutgoingRequest::EndpointError> From<FromHttpResponseError<Request>> for MatrixError {
// //     fn from(value: FromHttpResponseError<Request>) -> Self {
// //         MatrixError { error: Some(matrix_sdk::HttpError::Api(value)), goose_error: None }
// //     }
// // }
// // Request: OutgoingRequest + Debug,

// // impl<T> From<FromHttpResponseError<T>> for MatrixError {
// //     fn from(value: FromHttpResponseError<T>) -> Self {
// //         MatrixError { error: Some(value), goose_error: None }
// //     }
// // }
...
//     pub async fn register_event_handler<Ev, Ctx, H>(&self, handler: H) -> &GooseMatrixClient
//     where
//         Ev: SyncEvent + DeserializeOwned + Send + 'static,
//         H: EventHandler<Ev, Ctx>,
//         <H::Future as Future>::Output: EventHandlerResult,
...
//     /// [`room::Room`], [`Client`] for now.
//     // pub async fn register_notification_handler<H, Fut>(&self, handler: H) -> &Self
//     pub async fn register_notification_handler<H, Fut>(&self, handler: H) -> &GooseMatrixClient
//     where
//         // H: Fn(Notification, room::Room, Client) -> Fut
//         H: Fn(Notification, room::Room, GooseMatrixClient) -> Fut
//             + SendOutsideWasm
...
//     ) -> Result<MatrixResponse<login::v3::Response>, MatrixError<Error>> {
...
//     ) -> impl Stream<Item = crate::matrix::error::Result<SyncResponse>> + 'a {
//         let mut last_sync_time: Option<Instant> = None;
