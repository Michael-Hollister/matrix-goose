...
// impl ClientBuilder {
impl GooseClientBuilder {
    // pub(crate) fn new() -> Self {
    pub(crate) fn new(goose_user_index: usize) -> Self {
        Self {
            homeserver_cfg: None,
            http_cfg: None,
    ...
    pub async fn build(self) -> Result<GooseMatrixClient, ClientBuildError> {
        ...
        let inner = Arc::new(ClientInner {
            ...
            goose_user_index: self.goose_user_index,
        });

        debug!("Done building the Client");

        // Ok(Client { inner })
        Ok(GooseMatrixClient { inner })
    }
}
